...
export async function handleReservationSubmit(event) {
    ...
    try {
        ...
        if (data.reservationId) {
            ...
            // Update last assigned spot for this floor
            lastAssignedSpots[selectedFloor] = nextSpot;

            // Update floor stats immediately
        ...
        form.reset();
        showSuccessMessage(`Reservation created successfully! Spot ${selectedFloor}-${nextSpot} has been assigned.`);
        await loadUserReservations();
        updateAllFloorStats();
...
export async function loadUserReservations() {
    try {
        ...
        // Reset floor occupancy and cache
        reservationsCache.clear();
        FLOORS.forEach(floor => {
            floorOccupancy[floor].clear();
        });

        // Update cache and floor occupancy
        reservations.forEach(reservation => {
            // Ensure floor and spot number are properly set
            if (!reservation.floor || !reservation.spotNumber) {
                console.warn('Reservation missing floor or spot number:', reservation);
                return;
            }
            ...
            const validatedReservation = {
                ...
                spotNumber: parseInt(reservation.spotNumber)
            };
            
            reservationsCache.set(reservation.reservationId, validatedReservation);
            floorOccupancy[validatedReservation.floor].add(validatedReservation.spotNumber);
        });
...
export { reservationsCache, floorOccupancy };
